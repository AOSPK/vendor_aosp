...
func init() {
	android.RegisterModuleType("kraken_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = krakenExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = krakenExpandVariables(ctx, dep_file)
	...
	cmd := krakenExpandVariables(ctx, String(g.properties.Cmd))
