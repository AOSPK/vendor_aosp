...
cat <<EOF
Additional Kraken functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- krakengerrit:   A Git wrapper that fetches/pushes patch from/to Kraken Gerrit Review.
- krakenrebase:   Rebase a Gerrit change and push it again.
- krakenremote:   Add git remote for Kraken Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for Kraken Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Kraken-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function krakenremote()
{
    ...
    git remote rm kraken 2> /dev/null
    ...
    local KRAKEN_USER=$(git config --get gerrit.gerrit.aospk.org.username)
    if [ -z "$KRAKEN_USER" ]
    then
        git remote add kraken ssh://gerrit.aospk.org:29418/$PFX$PROJECT
    else
        git remote add kraken ssh://$KRAKEN_USER@gerrit.aospk.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'kraken' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/AOSPK/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        krakenremote
        git push kraken HEAD:refs/heads/'$1'
    fi
    '
}

function krakengerrit() {
    ...
    local user=`git config --get gerrit.gerrit.aospk.org.username`
...
function krakenrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Kraken Gerrit Rebase Usage: "
        echo "      krakenrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://gerrit.aospk.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
