...
def get_private_gerrit_url():
    cmd = ['git config --get review.gerrit-staging.aospk.org.username']
    username = subprocess.check_output(cmd, shell=True).decode("utf-8").strip()
    return 'ssh://{0}@gerrit-staging.aospk.org:29419'.format(username)

if __name__ == '__main__':
    # Default to Kraken Gerrit
    default_gerrit = 'https://gerrit.aospk.org'

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from Kraken's Gerrit instance (or any gerrit instance of your choosing)
